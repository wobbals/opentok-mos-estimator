...
#define SCORE_HEARTBEAT_INTERVAL_MS 15000
...
videoNetworkStatsUpdated:(OTSubscriberKitVideoNetworkStats*)stats
{
    ...
    __unused double score = [self calculateVideoScore];
    //NSLog(@"videoScore: %f", score);
    [self prune];
}
...
audioNetworkStatsUpdated:(OTSubscriberKitAudioNetworkStats*)stats
{
    ...
    __unused double score = [self calculateAudioScore];
    //NSLog(@"audioScore: %f", score);
    [self prune];
}
